...
**Adopt the figure generators instead of hand-building diagrams.** Before placing your own node boxes + connector lines, timeline rails, comparison columns, or step rows, call `recommend_figure` with the slide message (or `list_schematic_presets`) and use what it recommends. `generate_schematic` auto-fits each label inside its node, so card text never clips; `generate_native_diagram` routes every connector border-to-border, so arrows never dangle, pierce, or leave gaps. Reserve hand-built shape compositions for simple, short-label layouts (a few cards, a badge, an accent rule) — the moment a figure has multiple labeled nodes with sentence-length text, connectors between boxes, or 3+ comparable items, switch to the matching generator. `lint_deck` emits `diagram.native-connectors` when a connected diagram is hand-built; it is a warning for a small flow and a **blocking error once the flow is complex (4+ hand-placed connectors)**. Fix it by rebuilding with the generator, not by nudging coordinates — hand-routed arrows in a complex flow are exactly what produces dangling arrows and clipped node text.
