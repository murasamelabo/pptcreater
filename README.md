...
For finished-deck judgement, run `pptcreater quality-review <deck.json> --purpose-profile P?` or MCP `review_slide_quality`. This applies the ppptevaluater standard distilled from Japanese slide-design books: 9 dimensions (D1 message fit through D9 accessibility), purpose profiles P1-P5 with different weights/pass lines, anti-patterns A1-A6 (sparse, text-wall, photo-reliant, too plain, over-dense, over-decorated), and deck-story axes S1-S7. It complements `review_deck`: `review_deck` blocks deterministic render/content defects, while `quality-review` answers whether the deck is good for its purpose and returns top fixes for the next improvement loop.
