...
**Make sure the template is actually used.** A deck only inherits the real PowerPoint master/layouts when its `template` field references a **registered** template that carries the embedded package. Render surfaces two warnings (in the `warnings` array of `render_pptx` / `create_pptx` / `finalize`, and in the CLI render/finalize summary) when this goes wrong:
