...
- `info`: Information icon for notes or context.
- `plus`: Plus sign for adding or expansion.
- `minus`: Minus sign for removal or reduction.
- `x`: X mark for close, cancel, or negative states.
- `arrow-right`: Right arrow for flow or next step.
- `arrow-down`: Down arrow for vertical flows or next sections.
- `upload`: Upload arrow for import or publishing.
- `download`: Download arrow for export or retrieval.
- `link`: Link chain for connections and references.
- `cloud`: Cloud icon for cloud platforms or remote services.
- `database`: Database cylinder for storage or data sources.
- `server`: Server rack for compute or backend components.
- `user-group`: User group for audience, customers, or teams.
- `user`: Single user for a person, role, or customer.
- `lock`: Lock for security, access, and protected states.
- `key`: Key for authentication, access, or permissions.
- `settings`: Sliders for settings or tuning.
- `search`: Magnifying glass for search and discovery.
- `document`: Document page for files, policies, and reports.
- `folder`: Folder for collections and repositories.
- `table`: Grid table for matrix and comparison layouts.
- `tree`: Tree diagram for hierarchy or branching.
- `list`: List for ordered points and checklists.
- `layers`: Stacked layers for architecture or levels.
- `target`: Target for goals and focus areas.
- `flag`: Flag for milestones and priorities.
- `star`: Star for highlights or excellence.
- `heart`: Heart for care, value, or customer love.
- `home`: Home for base, landing, or location.
- `building`: Building for organizations and facilities.
- `globe`: Globe for global, web, and network contexts.
- `mail`: Envelope for email and messages.
- `calendar`: Calendar for schedules and dates.
- `clock`: Clock for time and duration.
- `bell`: Bell for notifications and alerts.
- `phone`: Phone for mobile or communication.
- `laptop`: Laptop for applications and workstations.
- `chart-up`: Rising chart for growth or improvement.
- `chart-bar`: Bar chart for metrics and comparisons.
- `pie-chart`: Pie chart for share or composition.
- `map`: Map for geography, routes, or coverage.
- `eye`: Eye for visibility and monitoring.
- `edit`: Pencil for editing or authoring.
- `trash`: Trash can for delete and cleanup.
- `filter`: Filter funnel for narrowing and segmentation.
- `code`: Code brackets for engineering and APIs.
- `branch`: Branching nodes for decisions and versioning.
- `cash`: Cash for cost, revenue, and financial topics.
- `scale`: Balance scale for governance and tradeoffs.
- `shield`: Shield for security, governance, or reliability.
- `lightbulb`: Lightbulb for ideas or insights.
- `workflow`: Connected nodes for process or architecture flows.
