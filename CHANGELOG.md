...
- Added a ppptevaluater-derived slide quality standard and review surface: first-pass rules now include D1-D9 dimensions, P1-P5 purpose profiles, A1-A6 anti-patterns, and S1-S7 deck-story checks; `review_visual_quality` flags obvious anti-patterns; and CLI/MCP expose `quality-review` / `review_slide_quality` for finished-deck scoring.
- Strengthened Message Map authoring so source-backed handout/technical decks preserve `details[]` and `sourceTrace[]`, and `from-message-map` / MCP `create_deck_from_message_map` fail fast when the Message Map is too thin before DeckSpec generation.
- Improved narrative-v1 table generation so non-key/value evidence is split into distinct label/body roles instead of repeating the same sentence in both columns.
- Improved narrative-v1 deck quality for technical/report handouts by recognizing generated table/native-diagram structure in quality scoring, refining repeated-layout detection, and treating intentionally fitted connector labels consistently with layout readability rules.
- Connected narrative-v1 Message Map generation to curated `design-packs/zukai` PowerPoint components so eligible slides can use richer full-slide flow, comparison, matrix, formula, step, and list patterns instead of defaulting to flat cards/tables; full-slide design components now avoid generated header overlays.
- Improved narrative-v1 technical diagrams so supplemental messages render as integrated bottom callouts, and zukai formula components preserve source-backed SSO/IdP/API authorization meaning instead of compressing it into ambiguous labels.
- Made zukai comparison and matrix components replace their full template vocabulary, including axis labels and table row headers, so content-specific decks do not retain unrelated plan-pricing terms such as monthly cost/support or generic effect/cost axes.
- Added PPTX-slide internal text-fit review for curated design components. `visual-review` / `review_visual_quality` now inspect transplanted `pptxSlide` template text boxes, flag long text placed into tiny icon/badge slots, and rendering compacts unsafe icon text to short markers so the full meaning can live in adjacent labels/captions instead of being clipped inside icons.
- Added design-component message sufficiency review: `visual-review` / `review_visual_quality` now warns when a transplanted `pptxSlide` has only compact markers and lacks visible explanatory text, and standalone `render_design_component` output now includes prose speaker notes explaining the slide message and reading path.
- Added a visual-review warning for large DeckSpec overlays placed on top of full-slide `pptxSlide` design components, steering authors to use existing template text slots, reserved panels, or speaker notes instead of covering the figure.
- Expanded figure selection guidance for MCP/CLI. `recommend_figure` / `pptcreater figure` now returns `useWhen`, `avoidWhen`, and `messageSpecDirection` (`slideRole`, `visualType`, and visual grammar) for every figure intent, including a new `text-explanation` / `detail-reading-page` path for text-first slides where prose is the main pattern.
...
- Added expressive Message Map archetypes for photo-led slides, large-number proof slides, chapter dividers, and concept diagrams so richer patterns are generated by the core tool rather than only by external scripts.
- Added `standaloneClarity` evaluation to the dev-loop so evaluators judge whether the visible slide output makes sense without reading generation scripts, scenario files, speaker notes, or hidden intent.
- Added whole-deck text completeness evaluation to the dev-loop so every visible text element, including cover titles and labels, is checked for complete understandable wording.
- Added mandatory per-slide written evaluator comments to dev-loop reports so every generated slide receives a concrete `would be better if` critique instead of relying only on numeric scores.
- Added an `editorial-board` Message Map archetype for `cards`, `detail`, and `visual-scaffold` intents, giving those scenarios a hero-claim plus supporting-card composition instead of falling back to generic statement panels.
- Added deterministic `source-check.txt` handling to the dev-loop so source-backed scenarios record an explicit source verification artifact or waiver instead of producing permanent residual PatchRequests.
- Promoted `layout.bad-line-break` findings from finalize/review into dev-loop PatchRequests and tightened Message Map label shortening so orphan continuations such as `を確認する。` are caught and avoided.
- Improved generated Message Map copy fitting so visible body text keeps complete sentence meaning and avoids clipped mid-word fragments.
