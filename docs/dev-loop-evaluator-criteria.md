...
この文書は、pptcreater 開発改善ループにおける `pptcreater Evaluator` が、どの条件で評価を開始し、何を証拠として扱い、どの基準で `FixRequest` を作るかを定義する。
...
| Deck artifacts | `deck.json`、polished DeckSpec、`.pptx`、review log、finalize log、Studio HTML、`visual-snapshots/contact-sheet.html`、per-slide SVG snapshots がある。 |
...
入力が不足している場合、Evaluator は評価を完了扱いにせず、`FixRequest` ではなく `reviewNotes` または `blockingIssues` として「証拠不足」を返す。
...
- `visual-snapshots/contact-sheet.html` または同等のレンダー画像証跡がなく、実見確認できない。
- scenario が WorkItem の acceptance と無関係で、評価しても開発判断に使えない。
- Evaluator にコード編集やDeckSpec修正を求めている。

## 評価軸

Evaluator は次の軸で評価する。各軸は 0-5 点で採点し、十分でない点は原則としてすべて `FixRequest` にする。FixRequest に severity は付けない。
...
| `sampleQuality` | サンプルスライド品質との距離 | 色、図解、ストーリー、図形構成、フォント/質感、テンプレート感の全てで、参照例に近いプロの資料に見える | 一部は改善しているが、まだテンプレ感や素人っぽい図形が残る | 色が野暮ったい、図解が代わり映えしない、ストーリーが伝わらない、フォントや質感が安っぽい |
...
加えて、`sampleQuality` では次を個別に見る。これらは「好み」ではなく、現在の成果物がサンプル品質から遠い主要因であるため、問題があれば `FixRequest` にする。

| 観点 | 見るもの | FixRequestにする状態 |
| --- | --- | --- |
| `colorSophistication` | 色の選び方、濁り、コントラスト、余白との相性 | 色が野暮ったい、薄い既定色の繰り返し、ブランド/主題に合わない |
| `diagramVariety` | 図解が内容ごとに変わっているか | どの資料でも同じカード/表/flowに見える |
| `storyClarity` | setup、tension、proof、action の流れ | スライド単体は読めても、資料全体で何を理解/判断するのか弱い |
| `shapeCraft` | 図形の組み方、主役/脇役、整列、プロっぽさ | 素人が四角とアイコンを並べたように見える |
| `typographyMateriality` | フォントサイズ差、太さ、質感、かすれ/安っぽさ | フォントが弱い、薄い、かすれた印象、階層が弱い |
| `templateFreshness` | テンプレート臭、既視感、シナリオ固有性 | どの資料でも代わり映えしないテンプレ感がある |

`expressionCraft` が 3 点未満の場合、Evaluator は `visual.expression-craft-low` の FixRequest を出す。証拠には、少なくとも「同一layoutの支配率」「写真/大きなvisualの有無」「空間モデル/大胆な反復/数値主役の有無」「どのサンプル由来の観点が欠けているか」を含める。

## レンダー画像の必須確認

Evaluator は DeckSpec のオブジェクト有無だけで合格判断してはならない。評価時は、少なくとも `visual-snapshots/contact-sheet.html` と各 `visual-snapshots/slide-XX.svg`、または同等のスクリーンショット/Studio/PPTX画像を確認する。

次は必ず `FixRequest` にする。

- 文字が枠からはみ出す、切れて見える、極端に小さい、読めない。
- テキスト同士、テキストと図形/画像、ラベルとアイコンが重なって読みにくい。
- 同じ小カードが並ぶだけで、視線の入口や主役が画像上で見えない。
- 数字やオブジェクトは存在するが、画像としては主張の主役になっていない。
- 色、余白、形、フォントの質感が野暮ったく、サンプルスライド的な完成度から遠い。

Evidence には、対象スライド番号、snapshot path、問題の見え方を入れる。例: `slide 4 visual-snapshots/slide-04.svg: label overlaps the focal card; rendered image is hard to read even though review has no blocking issue.`
...
| Gate | 評価条件 | 失敗時の扱い |
| --- | --- | --- |
| `finalize` | `Blocking errors: 0` である。 | FixRequest。render前に修正が必要。 |
| `review` | `Ready to finalize: no blocking issues` または blocking issue 0。 | FixRequest。owner と finding code を含める。 |
| zip integrity | zero-length non-directory entries が `0`。 | FixRequest。PPTX成果物として信用しない。 |
| build/test | code変更がある場合、focused tests と full test が通っている。 | FixRequest。開発側へ戻す。 |
| source/reference | 外部URLや公式情報を使う場合、metadata/source/reference slide または hyperlink が確認できる。 | FixRequest。出典不明なら customer-facing deck では必ず直す。 |

## FixRequest にする条件

Evaluator は、次の4点を満たす指摘を `FixRequest` として返す。表現品質の指摘も、資料の目的達成やサンプル品質との距離に影響する場合は修正対象にする。
...
単なる別案ではなく、成果物がサンプル品質から遠く見える理由を evidence と expected で説明できるなら `FixRequest` にする。例えば、色が野暮ったい、同じ表現が不自然に反復される、図形の組み合わせが素人っぽい、情報量と図解形式が合っていない、視線誘導が弱く判断アクションに結びつかない、といった問題は development loop に戻す。
...
次は `standaloneClarity` の FixRequest 対象にする。
...
Evaluator は severity を付けない。出力した `fixRequests` は原則すべて直す対象である。優先順位が必要な場合は QA / Dev Lead が作業順序として決めるが、Evaluator は「低いから直さなくてよい」という分類を作らない。

`fixRequests` はスライド個別修正ではなく、pptcreater 本体の改善要求として書く。対象範囲には、プログラムコード、プリセットテンプレート、図解生成、ポンチ絵生成、アイコン、デザインパック、フォント/色/余白ルール、agent guidance を含める。

## 証拠の書き方

`FixRequest.evidence` は、できるだけ次の形で書く。
...
```text
slide 4 uses a full-slide SVG image for a 5-node architecture diagram; ScenarioSpec.requiredTools includes generate_native_diagram, but tool ledger has no generate_native_diagram call. review log has no blocking issue, so this is a toolDiscipline/editability FixRequest rather than a finalize blocker.
```
