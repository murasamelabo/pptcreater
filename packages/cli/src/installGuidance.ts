...
8. Before building ANY multi-element figure (flow, process, timeline, comparison, hierarchy, cycle, matrix), call \`recommend_figure\` with the slide message (and/or \`list_schematic_presets\`) and follow its \`renderer\`. When it returns \`design-pack\`, PREFER \`render_design_component\` with a curated component of the recommended \`kind\` from the zukai pack (the 14 figure kinds: flow-horizontal, flow-vertical, cycle, before-after, matrix, venn, formula, comparison, scale, step, gantt, list-vertical, list-horizontal, list-enumeration, plus tree) — call \`list_design_components\` to pick a P1-P6 variant, use \`textReplacements\` to fill the eyebrow/title/labels/sub-labels/caption (replace every catalog placeholder), and \`nodeOperations\` to match the node count (the layout re-fits and renumbers). Do NOT change ○/△/✕ marks via text (they are colored icon shapes); keep the source mark pattern and map your columns onto it. Only when it returns \`schematic\` (no curated component) fall back to \`generate_schematic\` (auto-fits labels) or \`generate_native_diagram\` (routes connectors border-to-border). Only hand-build simple, short-label compositions (a few cards, a badge, an accent rule).
