...
8. Before building ANY multi-element figure (flow, process, timeline, comparison, hierarchy, cycle, matrix), call \`recommend_figure\` with the slide message (and/or \`list_schematic_presets\`) and adopt the recommended generator instead of hand-placing shapes. \`generate_schematic\` auto-fits label text so nodes never clip; \`generate_native_diagram\` routes connectors border-to-border so arrows never dangle. Only hand-build simple, short-label compositions (a few cards, a badge, an accent rule).
8. Before building ANY multi-element figure (flow, process, timeline, comparison, hierarchy, cycle, matrix), call \`recommend_figure\` with the slide message (and/or \`list_schematic_presets\`) and adopt the recommended generator instead of hand-placing shapes. \`generate_schematic\` auto-fits label text so nodes never clip; \`generate_native_diagram\` routes connectors border-to-border so arrows never dangle. Only hand-build simple, short-label compositions (a few cards, a badge, an accent rule).
...
- Adopt the figure generators (do not hand-build diagrams): before placing your own node boxes + connector lines, timeline rails, comparison columns, or step rows, call \`recommend_figure\` with the slide message (and/or \`list_schematic_presets\`) and use what it recommends. \`generate_schematic\` auto-fits each label inside its node so card text never clips (table/contrast/matrix for comparisons, tree/layer for hierarchy, flow/vertical-flow/cycle/step for processes, gantt for timelines, venn/set/puzzle/correlation/map for grouping). \`generate_native_diagram\` routes every connector border-to-border so arrows never dangle, pierce, or leave gaps, and auto-spaces nodes. \`generate_intent_diagram\` preserves a known composition. Hand-placing shapes for these is the main source of clipped node text and arrows that miss their boxes; reserve hand-built shape compositions for simple, short-label layouts (a few cards, a badge, an accent rule). The moment a figure has multiple labeled nodes with sentence-length text, connectors between boxes, or 3+ comparable items, switch to the matching generator. \`diagram.native-connectors\` warns on a hand-built connected diagram and becomes a blocking error once the flow is complex (4+ hand-placed connectors); fix it by rebuilding with the generator, not by nudging coordinates.
