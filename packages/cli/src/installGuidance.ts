...
- Visual richness gate: plain content slides should not be text-only. Use \`generate_intent_diagram\`, \`generate_native_diagram\`, \`generate_schematic\`, registered icons, images, or card/shape compositions so at least 75% of content slides have visual structure. Fix \`visual.richness-missing\` and \`visual.richness-deck\` before final output unless the slide is intentionally text-rich.
- Text-rich detail slides are allowed (the deck should not be ALL bullets/figures): for content that genuinely needs fuller prose — a detailed explanation, a Q&A / FAQ, a 得られること/benefits list with descriptions, or policy/decision text where reading is the point — call \`generate_detail_slide\` (variants \`explanation\`/\`qa\`/\`benefits\`) or use a \`detail\`/\`prose\`/\`structured-text\` layout. These layout markers are exempt from the visual-richness gate and excluded from the 75% denominator, so detailed paragraphs do NOT need a figure. Keep the title and any heading/label/question concise; put longer explanation in \`body\`/\`answer\`/\`description\`; use headings, indentation, bold emphasis, selective color, and whitespace so the slide does not become a black text block. AA contrast, reading order, overflow fitting, and concise-title checks still apply; detail-slide body copy may be 14-16pt. Keep these the exception — \`visual.prose-heavy\` warns when prose/Q&A slides outnumber the visual body slides.
