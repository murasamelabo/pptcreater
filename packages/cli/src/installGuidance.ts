...
- Visual richness gate: content slides must not be text-only. Use \`generate_native_diagram\`, \`generate_schematic\`, registered icons, images, or card/shape compositions so at least 75% of content slides have visual structure. Fix \`visual.richness-missing\` and \`visual.richness-deck\` before final output.
...
Use the pptcreater MCP for slide work. Start by clarifying purpose, audience, delivery mode, volume, and available source assets when they are unclear. Prefer editable PowerPoint shapes/text over flattened images, and use generate_native_diagram for architecture/security/ponchi-e diagrams. Run review_content and lint_deck before rendering, and use polish_deck_layout only when needed. Render with render_pptx/render_powerpoint, or CLI \`pptcreater render <deck.json> --output <deck.pptx> --polish\` if MCP render tools are not visible. Never use PowerPoint COM or ad-hoc PPTX scripts for normal output.
${CLAUDE_BLOCK_END}`;
}
