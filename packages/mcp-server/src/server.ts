          ...
          text: JSON.stringify(
            {
              ...
              templateFlow: "To actually USE a provided PowerPoint template (.pptx/.potx), not just imitate it: (1) import_template with register=true so its slide master/layouts/theme are embedded and saved to the registry; (2) START the deck with scaffold_from_template so the cover/closing reuse the template's OWN cover and the deck's `template` field references the registered id; (3) add content slides, then optionally apply_template_design to re-skin middle slides. Do NOT draw a custom hero/cover over a referenced template — it hides the template design. render_pptx/finalize_deck return warnings when this goes wrong: `template.package-not-embedded` means the real master was NOT embedded (the id is unregistered or lacks the package — re-run import_template register=true and reference that id), and `template.cover-overdrawn` means a generated hero/backdrop is hiding the template cover (rebuild that slide via scaffold_from_template).",
