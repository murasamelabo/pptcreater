          ...
          text: JSON.stringify(
            {
              ...
              figureAdoption: "ADOPT the figure generators instead of hand-building diagrams. Before placing your own node boxes + connector lines, timeline rails, comparison columns, or step rows, call recommend_figure with the slide message (and/or list_schematic_presets) and use what it recommends: generate_schematic for structured visuals (table/contrast/matrix for comparisons, tree/layer for hierarchy, flow/vertical-flow/cycle/step for processes, gantt for timelines/schedules, venn/set/puzzle/correlation/map for grouping) — it auto-fits each label inside its node so card text never clips; generate_native_diagram for connected architecture/flow/security ponchi-e — it routes every connector border-to-border so arrows never dangle, pierce, or leave gaps, and auto-spaces nodes when x/y are omitted; generate_intent_diagram when the conceptual composition is known. Hand-placing shapes for these is the main source of clipped node text and arrows that don't reach their boxes. Reserve hand-built shape compositions for simple, short-label layouts (a few cards, a badge, an accent rule); the moment a figure has multiple labeled nodes carrying sentence-length text, connectors between boxes, or 3+ comparable items, switch to the matching generator. lint emits diagram.native-connectors when a connected diagram is hand-built (an error once it is complex); fix it by rebuilding with the generator, not by nudging coordinates.",
