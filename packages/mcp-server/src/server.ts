          ...
          text: JSON.stringify(
            {
              ...
              cognitiveLoad: "Use one visual grammar per slide. Call list_schematic_presets before choosing a structured diagram kind. Prefer table/contrast for comparisons, tree/layer for hierarchy, flow/vertical-flow/cycle/step for processes, matrix/scale-contrast/grow/ranking for analysis, radar for multi-axis score profiles, gantt for schedules, venn/set/puzzle/correlation/map for conceptual grouping, generate_intent_diagram for known concept compositions/granularity, and generate_native_diagram for general architecture/security flows with editable connectors. Avoid many custom text boxes with uneven manual line breaks or body-only enumerations. Let layout polish wrap Japanese text instead of hand-coding line breaks. Content slides must not be text-only: fix visual.richness-missing and visual.richness-deck by adding generate_schematic, generate_intent_diagram, generate_native_diagram, registered icons, images, or card/shape composition so at least 75% of content slides have visual structure. When embedding SVG diagrams, keep internal labels at least 8pt after scaling and non-overlapping, or recreate/split them.",
