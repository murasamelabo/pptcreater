          ...
          text: JSON.stringify(
            {
              ...
              cognitiveLoad: "Use one visual grammar per slide. Call recommend_figure/list_schematic_presets before choosing a structured diagram kind, and treat table/contrast, tree/layer, flow/vertical-flow/cycle/step, matrix/scale-contrast/grow/ranking/radar, gantt, venn/set/puzzle/correlation/map, detail/prose, generated intent diagrams, native architecture diagrams, and source images as peer expression options. Select the grammar that best matches the slide message, data shape, and audience reading path. Avoid many custom text boxes with uneven manual line breaks or body-only enumerations. Let layout polish wrap Japanese text instead of hand-coding line breaks. Plain content slides should not be text-only: fix visual.richness-missing and visual.richness-deck by adding generate_schematic, generate_intent_diagram, generate_native_diagram, registered icons, images, or card/shape composition. Intentional text-rich slides are allowed when reading is the point: use detail/prose/structured-text, and reduce cognitive load with headings, indentation, bold emphasis, selective color, and whitespace. When embedding SVG diagrams, keep internal labels at least 8pt after scaling and non-overlapping, or recreate/split them.",
