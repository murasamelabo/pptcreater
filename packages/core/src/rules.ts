...
export function getSlideCreationRules(locale: Locale = "ja-JP", contentMode: ContentMode = "presentation"): SlideCreationRules {
  ...
  const layoutRules = locale === "ja-JP"
    ? [
        ...
        "図解はメッセージとデータ構造から選ぶ。比較には table/contrast、階層には tree/layer、工程には flow/vertical-flow/cycle/step、時間軸には timeline/gantt、分析には matrix/scale-contrast/grow/ranking/radar、システム全体像には architecture/native-diagram、集合・関係には venn/set/puzzle/correlation/map、3-4点要約には list/list-horizontal などを候補にし、最も読み筋が合う表現を採用する。"
      ]
    : [
        ...
        "Choose figures from the slide message and data structure. Consider comparison candidates (table/contrast), hierarchy candidates (tree/layer), process candidates (flow/vertical-flow/cycle/step), timeline/schedule candidates (timeline/gantt), analysis candidates (matrix/scale-contrast/grow/ranking/radar), system overview candidates (architecture/native-diagram), grouping/relationship candidates (venn/set/puzzle/correlation/map), and 3-4 point summary candidates (list/list-horizontal); adopt the visual grammar with the clearest reading path."
      ];
