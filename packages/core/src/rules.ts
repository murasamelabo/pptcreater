...
export function getSlideCreationRules(locale: Locale = "ja-JP", contentMode: ContentMode = "presentation"): SlideCreationRules {
  ...
  const visualRules = locale === "ja-JP"
    ? [
        ...
        "Slidelandのcool/minimal/trust系事例をベンチマークにし、表現力、フォント階層、配置、間隔、余白、配色が近いかを確認する。coolは強いコントラストと大胆な余白、minimalは削ぎ落とした構造と少色、trustは落ち着いた配色・安定グリッド・読みやすい表/グラフを重視する。",
    ...
    : [
        ...
        "Benchmark visual taste against Slideland-style cool/minimal/trust references: expression quality, typography hierarchy, placement, spacing, whitespace, and color discipline. Cool favors high contrast and bold whitespace; minimal favors reduction and few colors; trust favors calm palettes, stable grids, and legible tables/charts.",
