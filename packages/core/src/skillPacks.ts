...
export const BUILTIN_SKILL_PACKS: SkillPack[] = [
  ...
  },
  {
    ...
    rules: [
      ...
      "構造を抽出する: 箇条書きの並列・対比・包含・順序・因果・時間軸・システム全体像のどれかを見極め、recommend_figure / list_schematic_presets で対応する図解（list/contrast/tree/flow・step/timeline/architecture/cycle/matrix）に落とす。ここが最も重要で難しい工程。箇条書きはヘッダー付きの四角グリッド（カテゴリ＝見出しセル＋項目セル）にして種類をタテヨコに整理する",
  ...
  },
  {
    ...
    rules: [
      ...
      "Extract structure: decide whether the bullets are parallel, contrast, containment, sequence, causation, timeline, or system overview, then map them to a figure via recommend_figure / list_schematic_presets (list/contrast/tree/flow-step/timeline/architecture/cycle/matrix). This is the most important and hardest step. Turn bullet lists into a labeled box grid (category = header cell + item cells) sorted into rows/columns",
